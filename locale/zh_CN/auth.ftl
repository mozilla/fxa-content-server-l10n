...
subscriptionCancellation-outstanding-content = 您的 { $productName } 订阅已经取消，最后一次付款日期将在 { $invoiceDateOnly }，金额为 { $invoiceTotal }。您的服务将持续到目前的计费周期结束为止，即 { $serviceLastActiveDateOnly }。
# Variables:
# $productName (String) - The name of the subscribed product, e.g. Mozilla VPN
