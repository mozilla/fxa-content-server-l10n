...
# $deletionDate - the date when the account will be deleted if the user does not take action to-reactivate their account
inactiveAccountSecondWarning-impact = Zaradi nedejavnosti bo vaš račun in osebni podatki <strong>{ $deletionDate }</strong> trajno izbrisani.
...
lowRecoveryCodes-subject-2 =
    { $numberRemaining ->
        [0] Ni več rezervnih overitvenih kod
        [one] Ostaja samo 1 rezervna overitvena koda
        [two] Imate samo še { $numberRemaining } rezervnih overitvenih kod
        [few] Imate samo še { $numberRemaining } rezervnih overitvenih kod
...
passwordChangeRequired-sign-in-3 = Za ponovno prijavo v račun morate samo ponastaviti geslo.
...
password-forgot-otp-title = Ali ste pozabili geslo?
...
recovery-subject = Ponastavite geslo
...
verifyPrimary-action-2 = Potrdite e-poštni naslov
...
verifySecondaryCode-action-2 = Potrdite e-poštni naslov
# Variables:
