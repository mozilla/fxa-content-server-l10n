...
subscriptionCancellation-outstanding-content = Smy waš abonement { $productName } wotskazali. Waše kónčne płaćenje { $invoiceTotal } budźe so { $invoiceDateOnly } płaćić. Waša słužba hač do kónca wašeje aktualneje wotličenskeje doby dale dźe, tuž do { $serviceLastActiveDateOnly }.
# Variables:
# $productName (String) - The name of the subscribed product, e.g. Mozilla VPN
